...
export const updateLesson = (lessonId: number, data: {
  lessonTitle?: string;
  lessonType?: 'VIDEO' | 'DOCUMENT' | 'QUIZ' | 'ASSIGNMENT';
  lessonOrder?: number;
  contentUrl?: string;
...
export const getLessonsByModule = (moduleId: number) => 
...
export const updateLessonProgress = (lessonId: number, data: {
...
export const getModuleProgress = (moduleId: number) => 
  api.get(`/api/lessons/module/${moduleId}/progress`);
