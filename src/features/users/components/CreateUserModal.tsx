...
interface CreateUserModalProps {
    open: boolean;
    onClose: () => void;
    editing?: any; // User data for editing
...
export default function CreateUserModal({ open, onClose, editing, onSubmit, onCancel, isSubmitting }: CreateUserModalProps) {
    ...
    const [userRoles, setUserRoles] = useState<UserRole[]>([{ id: '1', role: '', center: '' }]);

    // Populate form data when editing
    useEffect(() => {
        if (editing) {
            setFormData({
                fullName: editing.name || 'Nguyễn Văn A',
                email: editing.email || 'email@education.vn',
                phone: editing.phone || '090xxxxxxx',
                dateOfBirth: '',
                gender: 'Nam',
                idCard: '123456789012',
                startDate: '',
                specialization: editing.major || 'Giáo dục',
                experience: editing.exp || '5 năm',
                address: '123 Đường ABC, Phường XYZ, Quận QWE',
                city: 'TP.HCM',
                district: 'Quận 1',
                ward: 'Phường ABC',
    ...
    const handleRoleChange = (index: number, field: 'role' | 'center', value: string) => {
        setUserRoles((prev) => prev.map((role, i) => (i === index ? { ...role, [field]: value } : role)));
    };
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (pickedGlobal.length > 0) {
            if (userRoles.length > 1) {
                toast.error(
                    'Lỗi xác thực',
                    'Role độc quyền (Super Admin / Quản lý đào tạo) không được đi kèm vai trò khác',
                );
                return;
            }
        } else {
            ...
            if (userRoles.length > 3) {
                toast.error('Lỗi xác thực', 'Tối đa 3 vai trò center-scoped cho mỗi user');
                return;
            }
