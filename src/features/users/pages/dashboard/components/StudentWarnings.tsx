...
import { studentWarningsApi, type StudentWarning as StudentWarningType } from '@/shared/api/student-warnings';

interface StudentWarningUI extends StudentWarningType {
...
const severityActionLabels: Record<string, string> = {
    HIGH: 'Liên hệ ngay',
    MEDIUM: 'Nhắc nhở',
    LOW: 'Theo dõi',
};
...
export default function StudentWarnings({ onCountChange }: Props) {
    ...
    const [warnings, setWarnings] = useState<StudentWarningUI[]>([]);
    ...
    useEffect(() => {
        const loadWarnings = async () => {
            try {
                setLoading(true);
                const response = await studentWarningsApi.getWarnings(selectedCenterId || undefined);
                const warningsData = response.data || [];
                
                // Add action labels based on severity
                const warningsWithLabels: StudentWarningUI[] = warningsData.map(w => ({
                    ...w,
                    actionLabel: severityActionLabels[w.severity] || 'Xem chi tiết',
                }));
                
                setWarnings(warningsWithLabels);
                onCountChange?.(warningsWithLabels.length);
            } catch (error) {
                console.error('Error loading student warnings:', error);
                setWarnings([]);
                onCountChange?.(0);
            } finally {
                setLoading(false);
    ...
    return (
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
            ...
            {loading ? (
                <div className="px-6 py-10 text-center text-sm text-gray-500">Đang tải cảnh báo...</div>
            ) : (
